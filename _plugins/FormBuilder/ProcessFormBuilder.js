...
function ProcessFormBuilder() {
	...
	function setupEditForm() {
		...
		$("#_ProcessFormBuilderView").on('click', function(e) {
		...
		$formFields
			.on('change', formFieldsChangeEvent)
			.on('init', formFieldsChangeEvent);

		$("#_ProcessFormBuilderEmbed").on('click', function() {
		...
		$("#_ProcessFormBuilderExport").on('click', function() {
			$.get("../exportForm?id=" + $("#form_id").val(), function(data) {
				$("#ProcessFormBuilderExportJSON").val(data).on('click', function() {
					$(this).select()
				});
			});
		});
		...
		if($("#fieldsetActions").length > 0) {
			$("#wrap_Inputfield_form_saveFlags").find('input').on('change', saveFlagsChangeEvent);
	...
	function setupEntries() {
		...
		if($action.length) {
			// PW 3.0.180+ (not currently used, maybe later)
			$('#entries_list_form').on('submit', function() {
				...
				if(numChecked) {
					...
					var actionText = $.trim($a.text()); 
					confirmText = confirmText.replace('%s', actionText).replace('%d', numChecked);
			...
			// PW 3.0.179 or prior (currently used for all versions)
			$(document).on('click', '.pwfb-confirm-submit', function() {
				...
				if(numChecked) {
					var confirmText = numChecked === 1 ? $form.attr('data-confirm-singular') : $form.attr('data-confirm-plural');
					var actionText = $.trim($button.text());
					confirmText = confirmText.replace('%s', actionText).replace('%d', numChecked);
					ProcessWire.confirm(confirmText, function() {
						$button.addClass('pwfb-action-confirmed').trigger('click');
					});
				} else {
					ProcessWire.alert($form.attr('data-none-checked'));
				}
		...
		if($closeLink.length) {
			...
			if($closeLink.hasClass('pwfb-close-now')) $closeLink.trigger('click');
		}
