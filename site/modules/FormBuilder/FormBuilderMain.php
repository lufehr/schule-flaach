...
class FormBuilderMain implements \IteratorAggregate, \Countable {
	 ...
	 *
	 */
	const RCD = 'ProcessWire Form Builder - Copyright 2023 by Ryan Cramer Design, LLC';
	...
	public function addNew($formName) {
		...
		$form->name = $formName; 
		$this->formNames[$form->id] = $formName;
		return $form; 
	}
	...
	public function entryToForm($entry) {
		...
		$query = $this->database->prepare('SELECT forms_id FROM forms_entries WHERE id=:id');
	...
	public function getEmbedVersion($v = '') {
		return $this->forms()->getEmbedVersion($v);
	}
	...
	public function helperPath($for, $item = '') {
		$config = $this->config;
		$forms = $this->forms();
		$helperDir = $forms ? $forms->get($for . 'Dir') : '';

		// attempt to locate in custom configured directory
		if($helperDir) {
			$helperPath = $config->paths->root . ltrim($helperDir, '/'); 
			if($item) $helperPath .= "$item/";
			if(is_dir($helperPath)) return $helperPath;
		}
		
		// attempt to locate in /site/templates/FormBuilder/themes/
		$path = $config->paths->templates . "FormBuilder/$for/";
		if($item) $path .= "$item/";
		if(is_dir($path)) return $path;

		// attempt to locate in /site/modules/FormBuilder/themes/	
		$path = $config->paths->get('FormBuilder') . "$for/";
		$_path = $path;
		if($item) $path .= "$item/";
		if(is_dir($path)) return $path;
	...
	public function helperURL($for, $item = '') {
		...
		// attempt to locate in custom configured directory
		if($helperDir) {
			$helperPath = $config->paths->root . ltrim($helperDir, '/') . ($item ? "$item/" : "");
			$helperUrl = $config->urls->root . ltrim($helperDir, '/') . ($item ? "$item/" : ""); 
			if(is_dir($helperPath)) return $helperUrl;
		}
		...
		if($item) $path .= "$item/";
		if(is_dir($path)) {
			$url = $config->urls->templates . "FormBuilder/$for/";
			if($item) $url .= "$item/";
			return $url;
		}
		...
		// attempt to locate in /site/modules/FormBuilder/themes/
		if($item) {
			$url = $config->urls->get('FormBuilder');
			$path = $config->paths->get('FormBuilder') . "$for/$item/";
			if(!is_dir($path)) {
		...
		return $config->urls->get('FormBuilder') . "$for/";
	}
	...
	public function error($str) {
		$obj = $this->forms();
		if(!$obj) $obj = wire('modules')->get('FormBuilder');
		if(!$obj) $obj = $this->wire;
		if($obj) $obj->error($str);
	}
