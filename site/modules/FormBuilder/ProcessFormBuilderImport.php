...
class ProcessFormBuilderImport extends Wire {
	...
	protected function buildImportProperties(FormBuilderForm $form, InputfieldCheckboxes $checkboxes, array $newData) {
		...
		foreach($newData as $key => $newValue) {
			...
			$oldValue = $this->previewValueString($oldValue);
			$newValue = $this->previewValueString($newValue);
	...
	/**
	 * Format value for output in diff preview
	 * 
	 * @param mixed $value
	 * @return string Returns entity encoded string
	 * 
	 */
	protected function previewValueString($value) {
		if(is_array($value)) {
		...
		foreach($importChildren as $value) {
			...
			if($field) {
				...
				foreach(array_reverse($parentNames) as $parentName) {
					$parent = $form->getFieldByName($parentName);
					if($parent) break;
				}
