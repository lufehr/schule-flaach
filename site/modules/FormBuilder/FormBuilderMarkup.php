...
class FormBuilderMarkup extends WireData {
	...
	public function __construct($markup, InputfieldForm $form, FormBuilderFramework $framework, $texts = array()) {
		...
		if($hasJqueryUI) {
			...
			$path = $forms->themesPath($fbForm->theme) . 'jquery-ui.css';
			if(is_file($path)) $styles[] = $forms->themesURL($fbForm->theme) . 'jquery-ui.css';
		}
		...
		$out .= "\n" . 
			"\n~?php if(\$submitted): /* When form submitted, show success message or redirect */ ?~" . 
			"\n" . 
			"\n\t~?php /* To redirect use: \$session->redirect('/url/to/success/page/'); */ ?~" .
			"\n\t~?php /* To display success message use the following: */ ?~" . 
			"\n\t<div id=\"FormBuilderSubmitted\">" . 
			"\n\t\t<h3>~?php echo \$successMessage; /* or replace with your own success message */ ?~</h3>" .
			"\n\t</div>" . 
	...
	public function testHash($testHash) {
		$config = $this->wire()->config;
		$testFile = $config->paths->templates . "FormBuilder/form-{$this->form->name}.php";
		$testURL = $config->urls->templates . "FormBuilder/form-{$this->form->name}.php";
