 ...
 * @method void exportCSV(FormBuilderForm $form, $selectorString, $filename = 'export.csv', $delimiter = ',', $useBOM = false)
 * @method string exportCSV_formatValueArray(FormBuilderForm $form, FormBuilderField $field, array $value)
...
class FormBuilderEntries extends Wire {
	...
	protected function wakeupEntry(array $row, $getCol = '') {
		...
		if(!empty($row['forms_id']) && $row['forms_id'] != $this->forms_id) {
			$form = $this->forms()->load((int) $row['forms_id']); 
		} else {
			$form = $this->getForm();
		}
	...
	protected function buildFindQuery($selectorString, $allForms = false) {
		...
		foreach($selectors as $selector) {	
			...
			switch($field) {
				...
				case 'entryFlags':
					...
					} else if(in_array($operator, array('>', '>=', '<', '<='), true) && $database->isOperator($operator)) {
						$query->where("flags$operator?", $value);
					} else {
				...
				case 'modified':
					...
					if(!ctype_alpha($field)) $field = 'created';
		...
		// force exclusion of partial and spam, if no flags appeared in selector
		if($flags === null) $query->where('flags=0');
	...
	protected function buildFindQueryField(DatabaseQuerySelect $query, $field, $operator, $value, $allForms = false, $not = false) {
		...
		if($inputfield instanceof InputfieldPage) $operator = '~=';
	 ...
	 * @param array $options
	 * @throws FormBuilderException|\PDOException
	 * @return array
	 * @since 0.5.2
	 * 
	 */
	...
	public function ___exportCSV(FormBuilderForm $form, $selectorString, array $options = array()) {
		...
		while($row = $query->fetch(\PDO::FETCH_ASSOC)) { 
			...
			foreach($fields as $name) {
				...
				if(array_key_exists($name, $formBuilderFields)) {
				...
				if($name === '_savePage' && $hasPagesPath) {
					if(empty($value)) {
						$value = '';
					} else {
					...
					$f = $formBuilderField ? $formBuilderField : $name;
					$value = $this->exportCSV_formatValue($form, $f, $value, $entry);
				}
	 ...
	 * @param FormBuilderForm $form
	 * @param FormBuilderField|string $field field or field name
	 ...
	 * 
	 */
	protected function exportCSV_formatValue(FormBuilderForm $form, $field, $value, &$entry) {
	
		if(!$field instanceof FormBuilderField) {
			// custom data: $field is field/property name
			...
			foreach($value as $k => $v) {
				...
				$fileURL = $this->forms()->getFileURL($form->id, $entry['id'], $file);
				if($fileURL === false) {
		...
		if(is_array($value)) {
			$value = $this->exportCSV_formatValueArray($form, $field, $value);
		}
	 ...
	 * 
	 */
	protected function ___exportCSV_formatValueArray(FormBuilderForm $form, $field, array $value) {
		if(empty($value)) return '';
		$hasArrays = false;
		foreach($value as $v) {
			if(is_array($v)) $hasArrays = true;
			if($hasArrays) break;
		}
		if($hasArrays) {
			$flags = JSON_PRETTY_PRINT;
			if(defined('JSON_UNESCAPED_UNICODE')) $flags = $flags | constant('JSON_UNESCAPED_UNICODE');
			if(defined('JSON_UNESCAPED_SLASHES')) $flags = $flags | constant('JSON_UNESCAPED_SLASHES');
	...
	public function ___deleteOlderThan($age, $ageType = 'DAYS', $dateColumn = 'created', $flags = null) {
		...
		if($qty) {
			$this->forms()->formLog($this->forms_id, "Deleted $qty entries older than $age $ageType old ($qtyFiles had files)"); 
		}
	...
	public function ___getFilesPath($entryID = 0, $create = true, $formID = 0) { 
		...
		$path = $this->forms()->getFilesPath(false, $create) . 'form-' . $formID . '/';
	...
	public function getForm() {
		...
		$this->form = $this->forms()->load($this->forms_id); 
		return $this->form;
	}
	...
	public static function _install($database) {
		...
		$createTable =  
			"CREATE TABLE " . self::entriesTable . " (" . 
				...
				"INDEX forms_id_flags_created (forms_id, flags, created), " . 
				"INDEX id_str (id, str), " . 
				"FULLTEXT `data` (`data`)" . 
